import { AIPrediction, ChannelType } from '../types';
 ...
 * Matches keywords to return highly reasonable triage metadata for custom user inputs
 */
export function heuristicClassify(channel: ChannelType, sender: string, messageText: string): AIPrediction {
  const text = messageText.toLowerCase();
  
  // 1. Check if it matches one of our exact sample messages
...
export async function classifyMessageWithGroq(
  ...
  if (!apiKey || apiKey.trim() === '') {
    throw new Error('API key is missing. Please provide a valid Groq API Key.');
  }
  ...
  try {
    ...
    // Validate types and default fallback if keys missing
    return {
      priority: (['urgent', 'normal', 'low'].includes(parsed.priority) ? parsed.priority : 'normal') as AIPrediction['priority'],
      leadQuality: (['strong', 'weak', 'unclear'].includes(parsed.leadQuality) ? parsed.leadQuality : 'unclear') as AIPrediction['leadQuality'],
      review: typeof parsed.review === 'boolean' ? parsed.review : false,
      category: parsed.category || 'General Classification',
      reasoning: parsed.reasoning || 'Classified via Llama 3 on Groq.',
      reply: parsed.reply || 'This message needs a human response — no auto-reply drafted.'
    };
  } catch (error: any) {
    console.error('Groq Classification failed, falling back to heuristic:', error);
    throw error; // Let outer handle the error and ask for fallback option
  }
}

/**
 * Live Groq API Reply Drafting Function (on-demand)
 */
...
Return ONLY a JSON object with exactly one field:
{
  ...
  try {
    ...
    console.error('Groq draft reply failed:', error);
    throw error;
  }
}
